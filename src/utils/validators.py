...
def validate_uploaded_file(file):
    """Validate uploaded file size and type (PDF, PNG, JPG, JPEG)."""
    if not file:
    ...
    if file.type not in allowed_types:
        return False, "Invalid file type. Please upload a PDF or an Image (PNG, JPG, JPEG)"
...
def validate_report_content(text):
    ...
    medical_terms = [
        ...
        'hemoglobin', 'wbc', 'rbc', 'platelet', 'glucose', 'creatinine', 'alt', 'ast'
    ]
    
    # Validate minimum text length (more lenient for smaller report screenshots)
    if not text or len(text.strip()) < 20:
        return False, "Extracted text is too short. Please ensure the file contains valid text."
